...
func GenerateKeyPair(bits int, privateKeyFile string, publicKeyFile string) (err error) {
	...
	if err != nil {
		err = errors.New("生成RSA密钥对时发生错误")
		return
	}

	err = SavePrivateKeyToFile(privateKey, privateKeyFile)
	if err != nil {
		err = errors.New("保存私钥文件时发生错误")
		return
	}
...
func DecryptPKCS1v15(privateKeyFile, ciphertext string) (plaintext string, err error) {
	...
	if err != nil {
		err = errors.New("读取私钥文件时发生错误:" + err.Error())
		return
	}

	privateKey, err := ParsePrivateKeyFromPEM(privateKeyBytes)
	if err != nil {
		err = errors.New("解析私钥时发生错误:" + err.Error())
		return
	}
	...
	if err != nil {
		err = errors.New("解密数据时发生错误:" + err.Error())
		return
	}
...
func SavePrivateKeyToFile(privateKey *rsa.PrivateKey, filename string) (err error) {
	...
	if err != nil {
		err = errors.New("保存私钥文件时发生错误:" + err.Error())
		return
	}
	return
}
...
func SavePublicKeyToFile(publicKey *rsa.PublicKey, filename string) (err error) {
	...
	if err != nil {
		err = errors.New("保存公钥文件时发生错误:" + err.Error())
		return
	}
	return
}
...
func ParsePrivateKeyFromPEM(pemBytes []byte) (privateKey *rsa.PrivateKey, err error) {
	...
	if !ok {
		err = errors.New("解析私钥时发生错误:invalid public key type")
		return
	}
	return
}
...
func ParsePublicKeyFromPEM(pemBytes []byte) (publicKey *rsa.PublicKey, err error) {
	...
	if !ok {
		err = errors.New("解析公钥时发生错误:invalid public key type")
		return
	}
...
func EncryptPassword(password string, salt string) (string, error) {
	password, err := EncryptString(password)
	if err != nil {
		return "", err
	}
	salt, err = EncryptString(salt)
	if err != nil {
		return "", err
	}
...
func EncryptString(data string) (encrypt string, err error) {
	...
	if _, err = h.Write([]byte(data)); err != nil {
		err = errors.New(`hash.Write failed` + err.Error())
		return "", err
	}
