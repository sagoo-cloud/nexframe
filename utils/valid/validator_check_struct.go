...
func (v *Validator) doCheckStruct(ctx context.Context, object interface{}) Error {
	...
	if v.assoc == nil || !v.useAssocInsteadOfObjectAttributes {
		...
		inputParamMap = convert.Map(v.assoc)
	}
	...
	for _, field := range tagFields {
		...
		if _, ok := inputParamMap[name]; !ok {
			if !v.useAssocInsteadOfObjectAttributes {
				...
				if name != fieldName {
					if foundKey, foundValue := utils.MapPossibleItemByKey(inputParamMap, fieldName); foundKey != "" {
	...
	for _, checkRuleItem := range checkRules {
		...
		if value != nil {
			...
			case reflect.Struct, reflect.Map:
				if convert.String(value) == emptyJsonObjectStr {
					value = ""
				}
			case reflect.Slice, reflect.Array:
				if convert.String(value) == emptyJsonArrayStr {
					value = ""
				}
			default:
			}
		...
		if validatedError := v.doCheckValue(ctx, doCheckValueInput{
			...
			// ============================================================
			if !checkRuleItem.IsMeta && (value == nil || convert.String(value) == "") {
				required := false
				// rule => error
...
func getPossibleValueFromMap(inputParamMap map[string]interface{}, fieldName, aliasName string) (value interface{}) {
	_, value = utils.MapPossibleItemByKey(inputParamMap, fieldName)
	if value == nil && aliasName != "" {
		_, value = utils.MapPossibleItemByKey(inputParamMap, aliasName)
	}
	return
}
