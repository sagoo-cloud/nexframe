...
func (v *Validator) doCheckValue(ctx context.Context, in doCheckValueInput) Error {
	...
	default:
		for k, message := range convert.Map(in.Messages) {
			customMsgMap[k] = convert.String(message)
		}
	}
	...
	for index := 0; index < len(ruleItems); {
		var (
			...
			ruleKey     = Trim(results[1])                               // rule key like "max" in rule "max: 6"
			rulePattern = Trim(results[2])                               // rule pattern is like "6" in rule:"max:6"
		)
		...
		if hasForeachRule {
			...
			foreachValues = convert.Interfaces(in.Value)
		...
		for _, value := range foreachValues {
			...
			case customRuleFunc != nil:
				err = customRuleFunc(ctx, RuleFuncInput{
					...
					Value:     convert.Any(value),
					Data:      convert.Any(in.DataRaw),
				})
			...
			if err != nil {
				...
				if errMsg := err.Error(); strings.Contains(errMsg, "{") {
					errMsg = utils.ReplaceByMap(errMsg, map[string]string{
						"{field}":     in.Name,               // Field name of the `value`.
						"{value}":     convert.String(value), // Current validating value.
						"{pattern}":   rulePattern,           // The variable part of the rule.
						"{attribute}": in.Name,               // The same as `{field}`. It is deprecated.
					})
					errMsg, _ = utils.ReplaceString(`\s{2,}`, ` `, errMsg)
...
func (v *Validator) doCheckValueRecursively(ctx context.Context, in doCheckValueRecursivelyInput) {
	...
	case reflect.Slice, reflect.Array:
		...
		if json.Valid(convert.Bytes(in.Value)) {
			array = convert.Interfaces(convert.Bytes(in.Value))
		} else {
			array = convert.Interfaces(in.Value)
		}
		if len(array) == 0 {
			return
		}
